...
const DEFAULT_GROQ_MODEL = "qwen/qwen3.8-27b"; // current vision-capable model — meta-llama/llama-4-scout was deprecated by Groq on 2026-07-17
...
async function generateWithGroq({ apiKey, model, product, imageUrls, promptConfig, designers, models }) {
  ...
  const imageParts = [];
  for (const url of imageUrls.slice(0, 3)) { // qwen3.8-27b on Groq accepts max 3 images per request
    try {
