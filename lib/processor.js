...
async function runBatch({ shopify, config, mode, limit, runLog, deadlineMs = Infinity }) {
      ...
      for (const product of products) {
        ...
        try {
          ...
          if (err.dailyQuotaExhausted) {
            // Reaches here in two cases:
            //  1) Gemini's daily quota is exhausted AND no GROQ_API_KEY is
            //     configured to fall back to.
            //  2) We already switched to Groq earlier this run (geminiExhausted
            //     is true), and Groq's OWN daily quota just ran out too.
            // Either way: stop the whole run cleanly here rather than burning
            // through every remaining product as a "failure". The next
            // scheduled run will simply continue from here automatically.
            stopReason = "daily_quota";
            const providerNote = geminiExhausted
              ? "Groq's daily quota is also used up for now"
              : "Gemini's daily quota is used up for now (no GROQ_API_KEY configured to fall back to)";
            runLog.push(
              log(`${providerNote}. Stopping this run cleanly — it will resume automatically on the next scheduled run. (${err.message})`)
            );
            break;
          }
  ...
  // This runs for EVERY configured collection on EVERY run — including
  // collections already fully finished — so if new products land in a
  // collection you already completed (e.g. "Sapphire" going from 5,000 to
  // 5,010), it's detected and surfaced here, and drainQueue() above will
  // naturally pick those new (untagged) products up on the next run since
  // it re-scans the whole collection from scratch each time.
  if (collectionQueue.length > 0) {
    ...
    for (const handle of collectionQueue) {
      try {
        ...
        const previous = progress.collections[handle];
        const previousTotal = previous ? previous.total : stats.total; // first time seeing this collection: no delta yet
        const newInCollection = Math.max(0, stats.total - previousTotal);

        progress.collections[handle] = { ...stats, new_since_last_check: newInCollection };

        if (newInCollection > 0) {
          runLog.push(
            log(
              `${newInCollection} new product${newInCollection === 1 ? "" : "s"} detected in "${stats.name}" — will be processed automatically on an upcoming run.`
            )
          );
        }
      } catch (err) {
