...
class ShopifyClient {
  ...
  async _getAccessToken() {
    ...
    if (!res.ok) {
      ...
      const err = new Error(
        `Shopify token request failed (${res.status}): ${text}`
      );
    ...
    // Shopify client-credentials tokens are typically valid for 24h;
    // refresh a bit early.
  ...
  async graphql(query, variables = {}) {
    ...
    return retryWithBackoff(
      async () => {
        ...
        // Cost-based rate limiting: back off proactively if we're close
        // to the bucket limit.
        ...
        if (cost && cost.throttleStatus) {
          ...
          if (currentlyAvailable < maximumAvailable * 0.2) {
            log(
              "Close to Shopify GraphQL rate limit bucket — pausing briefly."
            );
   ...
   */
  async fetchUnprocessedProducts({
    first = 25,
    cursor = null,
    collectionHandles = [],
  } = {}) {
    ...
    if (collectionHandles && collectionHandles.length) {
      const collectionsPart = collectionHandles
        .map((h) => `collection:${h}`)
        .join(" OR ");
     ...
     * IMPORTANT:
     *
     * Shopify GraphQL returns product.media as a connection object:
     *
     * product.media = {
     *   nodes: [...]
     * }
     *
     * The processor expects product.media to be an array and uses
     * .map() and .slice() on it.
     *
     * Normalize the Shopify response here so the rest of the application
     * can continue using product.media as a normal array.
     */
  ...
  /**
   * Backup the original description into a metafield so it can always
   * be restored.
   *
   * Namespace/key:
   * custom.ai_backup_description
   */
  async backupOriginalDescription(productId, originalHtml) {
    ...
    if (errors && errors.length) {
      throw new Error(
        `Backup metafield failed: ${JSON.stringify(errors)}`
      );
    }
  }
  ...
  async updateProduct(
    ...
    if (errors && errors.length) {
      throw new Error(
        `productUpdate failed: ${JSON.stringify(errors)}`
      );
    }
  }
  ...
  async addTag(productId, tag) {
    ...
    if (errors && errors.length) {
      throw new Error(
        `tagsAdd failed: ${JSON.stringify(errors)}`
      );
    }
  }
  ...
  async updateMediaAltText(productId, mediaId, altText) {
    ...
    if (errors && errors.length) {
      throw new Error(
        `Alt text update failed: ${JSON.stringify(errors)}`
      );
    }
  }
}
