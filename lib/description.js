...
Also produce:
- "seo_title": an SEO-friendly product title (max 70 characters), incorporating product type + designer (if any) + one strong descriptive term. Do not change the core product identity.
...
async function generateDescription({ apiKey, model, product, attributes, promptConfig, designers, models }) {
  ...
  result.seo_title = truncate(cleanText(result.seo_title), 70);
